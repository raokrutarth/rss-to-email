...
object Firework:
  ...
  /** @return
    ...
    * `next` on the underlying state type (see e.g., [[Waiting.next]] ).
    *
...
case class Waiting(
  ...
  /** @return
    ...
    * all the next states.
    *
    * Hint: use the [[Launched.init]] operation to transition the firework to
    * the [[Launched]] state.
    */
  def next: Firework =
...
case class Launched(
  ...
  def next: Firework =
    if countDown > 0 then
      val new_position = Motion.movePoint(
        point = position,
        direction = direction,
        speed = Settings.propulsionSpeed
      )
      copy(
