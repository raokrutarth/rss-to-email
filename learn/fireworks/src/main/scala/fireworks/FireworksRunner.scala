...
/** Program entry point.
  *
  * Create 10 random fireworks and compute their evolution over time until they
  * are all done.
  *
...
@main def run(): Unit =
  ...
  val fireworksOverTime =
    Observable.unfold(initFireworks) { fireworks =>
      if fireworks.isEmpty then None
      else
  ...
  val frame = Frame
    .size(Settings.width.toDouble, Settings.height.toDouble)
