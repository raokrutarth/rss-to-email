...
object InMemoryModel extends Model:
  ...
  def complete(id: Id): Option[Task] =
    // if idStore.contains(id) then idStore(id) = idStore(id).complete
    idStore.updateWith(id)(opt =>
      opt match
        case Some(t) => Option(t.complete)
        case _       => None
    )
    idStore.get(id)
  ...
  def delete(id: Id): Boolean =
    ...
    if idStore.contains(id) then idStore.remove(id)
    found
  ...
  def tags: Tags =
    Tags(idStore.values.toSet.flatMap(t => t.tags).toList)

  def tasks(tag: Tag): Tasks =
    // idStore.filterInPlace((k, v) => v.tags.contains(tag))
